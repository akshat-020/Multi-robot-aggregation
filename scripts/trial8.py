...
def surr_data_tb0(data0):
        ...
        # omit the inf value
        p0=np.concatenate((p0[-90:-1],p0[0:90]),axis=None)
        ...
        minvalue_of_ranges0=np.argmin(p0)-90    ## have to find the index of the minimum value then find the degree deltaij
...
def surr_data_tb1(data1):
        ...
        p1=np.concatenate((p1[-90:-1],p1[0:90]),axis=None)
       
        for g in range(len(p1)):
            if p1[g]==float('inf'):
                p1[g]=3.5
            elif math.isnan(p1[g]):
                p0[g]=0
            else:
                continue
         #print(len(p1))
        minvalue_of_ranges1=np.argmin(p1)-90   ## have to find the index of the minimum value then      ## have to value then find the degree deltaij
...
def surr_data_tb2(data2):
        ...
        p2=np.concatenate((p2[-90:-1],p2[0:90]),axis=None)
        for g in range(len(p2)):
            if p2[g]==float('inf'):
                p2[g]=3.5
            elif math.isnan(p2[g]):
                p0[g]=0
            else:
                continue
        #print(len(p2))
        
        minvalue_of_ranges2=np.argmin(p2)-90   ## have to find the index of the minimum value then      ## have to then find the degree deltaij
