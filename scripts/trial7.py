...
def surr_data_tb0(data0):
        ...
        minvalue_of_ranges0=np.argmin(np.concatenate((p0[-35:-1],p0[0:35]),axis=None))-35    ## have to find the index of the minimum value then find the degree deltaij
...
def surr_data_tb1(data1):
        ...
        for g in range(len(p1)):
            if p1[g]==float('inf'):
                ...
                continue
        #print(len(p1))
        minvalue_of_ranges1=np.argmin(np.concatenate((p1[-35:-1],p1[0:35]),axis=None))-35     ## have to value then find the degree deltaij
...
def surr_data_tb2(data2):
        ...
        for g in range(len(p2)):
            if p2[g]==float('inf'):
                ...
                continue
        #print(len(p2))
        minvalue_of_ranges2=np.argmin(np.concatenate((p2[-35:-1],p2[0:35]),axis=None))-35    ## have to then find the degree deltaij
